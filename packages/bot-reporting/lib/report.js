...
controller.hears(['report', 'rapport'], ['direct_mention','direct_message'], Meteor.bindEnvironment((bot, message) => {
    slack.api('auth.test', { user: message.user }, Meteor.bindEnvironment((err, auth) => {
        slack.api('users.info', { user: message.user }, Meteor.bindEnvironment((err, user) => {
            ...
            let newProblem;
            let start = (err, convo) => {
                convo.say(i18n('welcome') + user.user.name + '! :simple_smile:');
                learn(message, convo);
            };
